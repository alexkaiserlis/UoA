...
# Updated path - looks like the jsonl files are already here
base_path = Path(".")  # Current directory since jsonl files are here
...
# Check if 'ner' column exists and get unique tags
if 'ner' in df.columns:
    all_tags = []
    for ner_list in df['ner']:
        if isinstance(ner_list, list):
            all_tags.extend(ner_list)
    unique_tags = list(set(all_tags))
    print(f"The final tagset (in IOB notation) is the following: `{unique_tags}`")
else:
    print("Warning: 'ner' column not found in DataFrame")
